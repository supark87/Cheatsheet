...
# 0.0 Mounting/unmounting  drives 

cd /Volume && ls		 	#list all volumes, then 
cd		 		 	#into desired volume 

diskutil list 	 			#lists all volumes 
diskutil mount [/dev/disk*]	 	#mounts volume
diskutil unmount  [/dev/disk*]		#unmounts volume
diskutil unmountDisk [/dev/disk*]	#if disk has partitioning scheme 
source 					#execute commands from filename 
					#same as . [filename] 
...
tar -options <filename>       # compress/decompress tar file; use tar --h for options
...
<label     # here-document;might have another < next to <label
...
trap returntrap RETURN  # is executed each time a shell function or a script executed with the . or  commands finishes executing
